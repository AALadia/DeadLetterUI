...
def createDeadLetter():
    if request.is_json:
        ...
        if isinstance(data["message"]["data"], str):
            data["message"]["data"] = PubSub().decodeMessage(
                data["message"]["data"])
        ...
        publisherProjectName = data["message"]["data"].get(
            'publisherProjectName')
        topicName = data["message"]["attributes"].get("topicName")
        subscriberName = data["subscription"]
        endpoint = ""
        errorMessage = ""
        publisherProjectId = data["message"]["attributes"].get(
            "publisherProjectId")
        publisherProjectName = data["message"]["attributes"].get(
            "publisherProjectName")
        originalMessage = data["message"]["data"]
    try:
        res = PubSubRequests().createDeadLetter(_id, originalMessage,
